  ...
  <div class="bg-rose-800">
    <nav class="flex items-center justify-between flex-wrap p-6 md:w-10/12 mx-auto">
      <div class="flex items-center flex-shrink-0 text-white mr-6">
        ...
        <router-link class="font-semibold text-xl tracking-tight" to="/">Ticket portál</router-link>
      </div>
...
export default defineComponent({
  setup() {
    ...
    let title: string = "Ahoj názov";

    const age: number = 22;

    const arr: number[] = [1, 6, 88, 99];

    let arrcokolvek: any[] = [1, "napdpis", true];

    let person: [number, string, boolean] = [1, "Odhlásil sa", true];
  },
});
</script>
