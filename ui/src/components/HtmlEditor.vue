<template>
  <div class="html-editor rounded-xl border border-slate-200 bg-white focus-within:border-blue-400 focus-within:ring-1 focus-within:ring-blue-300">
    ...
    <div class="flex flex-wrap items-center gap-0.5 border-b border-slate-200 p-1.5">
      <ToolBtn :title="t('editor.bold')" :active="editor.isActive('bold')" @click="editor.chain().focus().toggleBold().run()">
      ...
      <ToolBtn :title="t('editor.italic')" :active="editor.isActive('italic')" @click="editor.chain().focus().toggleItalic().run()">
      ...
      <ToolBtn :title="t('editor.underline')" :active="editor.isActive('underline')" @click="editor.chain().focus().toggleUnderline().run()">
      ...
      <ToolBtn :title="t('editor.h2')" :active="editor.isActive('heading', { level: 2 })" @click="editor.chain().focus().toggleHeading({ level: 2 }).run()">
        <span class="text-xs font-bold">H2</span>
      </ToolBtn>
      <ToolBtn :title="t('editor.h3')" :active="editor.isActive('heading', { level: 3 })" @click="editor.chain().focus().toggleHeading({ level: 3 }).run()">
        <span class="text-xs font-bold">H3</span>
      </ToolBtn>

      <div class="mx-1 h-5 w-px bg-slate-200" />

      <ToolBtn :title="t('editor.bulletList')" :active="editor.isActive('bulletList')" @click="editor.chain().focus().toggleBulletList().run()">
      ...
      <ToolBtn :title="t('editor.orderedList')" :active="editor.isActive('orderedList')" @click="editor.chain().focus().toggleOrderedList().run()">
      ...
      <ToolBtn :title="t('editor.link')" :active="editor.isActive('link')" @click="setLink">
      ...
      <ToolBtn v-if="editor.isActive('link')" :title="t('editor.unlink')" @click="editor.chain().focus().unsetLink().run()">
      ...
      <ToolBtn :title="t('editor.quote')" :active="editor.isActive('blockquote')" @click="editor.chain().focus().toggleBlockquote().run()">
        <svg class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor"><path d="M3 21c3 0 7-1 7-8V5c0-1.25-.756-2.017-2-2H4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2 1 0 1 0 1 1v1c0 1-1 2-2 2s-1 .008-1 1.031V20c0 1 0 1 1 1z"/><path d="M15 21c3 0 7-1 7-8V5c0-1.25-.757-2.017-2-2h-4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2h.75c0 2.25.25 4-2.75 4v3c0 1 0 1 1 1z"/></svg>
      </ToolBtn>
      <ToolBtn :title="t('editor.hr')" @click="editor.chain().focus().setHorizontalRule().run()">
        <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="5" y1="12" x2="19" y2="12"/></svg>
      </ToolBtn>

      <div class="mx-1 h-5 w-px bg-slate-200" />

      <ToolBtn :title="t('editor.undo')" :disabled="!editor.can().undo()" @click="editor.chain().focus().undo().run()">
      ...
      <ToolBtn :title="t('editor.redo')" :disabled="!editor.can().redo()" @click="editor.chain().focus().redo().run()">
...
// Placeholder sa vyhodnocuje až tu, nie v `withDefaults` — tá hodnota vzniká
// pri načítaní modulu, teda ešte pred voľbou jazyka.
