        ...
        <li v-for="message in messages" :key="message.id">
          <button type="button" class="msg-row" :class="{ active: activeId === message.id }"
            ...
            <span class="flex items-center gap-2">
              <span v-if="!message.readAt" class="size-2 shrink-0 rounded-full bg-teal-500" :aria-label="t('messages.unread')" />
      ...
      <section v-if="thread" class="rounded-2xl border border-slate-200 bg-white p-4">
        <header class="mb-3 flex flex-wrap items-start justify-between gap-2 border-b border-slate-100 pb-3">
          ...
          <button v-if="thread.permissions.markRead" type="button" class="action-btn"
            ...
            {{ thread.readAt ? t('messages.markUnread') : t('messages.markRead') }}
          </button>
        </header>

        <article v-for="entry in [thread, ...thread.replies]" :key="entry.id" class="mb-3">
          <p class="text-xs text-slate-400">
            {{ entry.outgoing ? t('messages.me') : entry.senderName }} · {{ formatDateTime(entry.createdAt) }}
          </p>
        ...
        <form v-if="thread.permissions.reply" class="mt-4 border-t border-slate-100 pt-3" @submit.prevent="sendReply">
          <FormField v-model="replyBody" type="textarea" rows="4" :placeholder="t('messages.replyPlaceholder')" />
          ...
          <div class="mt-2 flex items-center gap-2">
            ...
            <span class="text-xs text-slate-500">{{ t('messages.replyNote') }}</span>
          </div>
        </form>
        <p v-else class="mt-4 border-t border-slate-100 pt-3 text-sm text-slate-500">
          {{ thread.outgoing ? t('messages.outgoingNote') : t('messages.noReplyNote') }}
        </p>
      </section>
...
function formatDateTime(value: string) {
  return new Date(value).toLocaleString(currentLocale(), { day: 'numeric', month: 'numeric', hour: '2-digit', minute: '2-digit' })
}
...
async function sendReply() {
  ...
  try {
    ...
    replyError.value = resp?.message ?? t('messages.replyFailed')
  } finally {
