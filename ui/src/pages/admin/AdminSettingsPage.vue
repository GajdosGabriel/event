  ...
  <div class="grid gap-6">
    ...
    <section class="panel-card grid gap-4">
      ...
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <FormField v-model="draft.eventsPerPage" type="select" label="Eventy (admin)" :options="perPageOptions" />
        <FormField v-model="draft.venuesPerPage" type="select" label="Miesta (admin)" :options="perPageOptions" />
        <FormField v-model="draft.canalsPerPage" type="select" label="Kanály (admin)" :options="perPageOptions" />
        <FormField v-model="draft.publicEventsPerPage" type="select" label="Eventy (verejné)" :options="perPageOptions" />
      </div>
    ...
    <section>
      <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
        <h2 class="text-base font-semibold text-slate-700">Organizácie</h2>
        <RouterLink to="/admin/organizations" class="text-sm text-blue-700 no-underline">Spravovať →</RouterLink>
      </div>
      ...
      <div v-else class="panel-card">
        <ul class="grid gap-2">
          <li v-for="org in orgs" :key="org.id" class="flex items-center gap-3 rounded-lg border border-slate-200 p-3">
            ...
            <span v-if="org.accountUuid" class="rounded-full bg-green-50 px-2 py-0.5 text-xs font-medium text-green-700">v Accounte</span>
            ...
            <RouterLink :to="`/admin/organizations/${org.id}/edit`" class="action-btn">Upraviť</RouterLink>
...
const perPageOptions = PER_PAGE_OPTIONS.map(n => ({ value: n, label: `${n} / stránku` }))
